...
async function transactionsRoute(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (!req.session.user) {
    ...
    const transactions = await dynamicsTransaction(
...
export default withSessionRoute(transactionsRoute);
